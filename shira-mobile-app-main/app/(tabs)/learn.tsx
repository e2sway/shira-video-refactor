...
// Add the showSuperwall function for direct paywall presentation in the Learn screen
const showSuperwall = async (
    user: any,
    fromNextVideo: boolean = true,
    onSuccess?: () => void
): Promise<boolean> => {
    logEvent('SHOW_SUPERWALL', {
        userId: user?.id,
        fromNextVideo,
        timestamp: new Date().toISOString()
    });
    ...
    try {
        ...
        console.log('[Learn] Showing Superwall paywall for next video');
        
        // Prepare user attributes
        const userAttributes = {
            userId: user.id,
            email: user.email || 'unknown@email.com',
            displayName: user.display_name || (user.email ? user.email.split('@')[0] : 'User'),
            targetLanguage: user.target_lang || 'Spanish',
            isPro: user.is_pro || false,
            fromNextVideo: fromNextVideo,
        };
            ...
            Superwall.shared.setUserAttributes(userAttributes)
                .then(() => {
                    ...
                    Superwall.shared.register({
                        placement: 'next_video',
                        params: {
                            source: 'learn_tab',
                            context: 'next_video_swipe'
                        },
                        feature: async () => {
                            ...
                            console.log('[Learn] Feature callback executed - purchase successful or user already has access');
                            ...
                            try {
                                ...
                                // Optional: Double-check with RevenueCat to ensure everything is in sync
                                try {
                                    const { isPro } = await checkSubscriptionStatus();
                                    logEvent('REVENUECAT_STATUS_CHECK', { 
                                        userId: user.id,
                                        isPro
                                    });
                                } catch (revenueCatError) {
                                    logEvent('REVENUECAT_STATUS_CHECK_ERROR', { 
                                        userId: user.id,
                                        error: revenueCatError instanceof Error ? revenueCatError.message : String(revenueCatError)
                                    });
                                    // Continue even if this fails
                                }
                                
                                logEvent('USER_PRO_STATUS_UPDATED', { userId: user.id });
                                ...
                                logEvent('ERROR_UPDATING_USER_STATUS', { 
                                    userId: user.id,
                                    error: error instanceof Error ? error.message : String(error)
                                });
                                // Still call the success callback even if there was an error
                                if (onSuccess) {
                                    onSuccess();
                                }
                            }
                        ...
                        handler: {
                            onPresent: () => {
                                console.log('[Learn] Paywall presented');
                                logEvent('PAYWALL_PRESENTED', { userId: user.id });
                                // We consider this a successful presentation
                                resolve(true);
                            },
                            onDismiss: () => {
                                console.log('[Learn] Paywall dismissed');
                                logEvent('PAYWALL_DISMISSED', { userId: user.id });
                            },
                            onError: (error) => {
                                console.error('[Learn] Error presenting paywall:', error);
                                ...
                                Toast.show({
                                    type: 'error',
                                    text1: 'Error',
                                    text2: 'Failed to show subscription options. Please try again.',
                                    position: 'bottom'
                                });
                            ...
                            onSkip: (reason) => {
                                console.log(`[Learn] Paywall skipped, reason: ${String(reason)}`);
                                logEvent('PAYWALL_SKIPPED', { 
                                    userId: user.id,
                ...
                .catch(error => {
                    console.error('[Learn] Error setting user attributes:', error);
                    logEvent('SUPERWALL_ERROR', { 
                        error: error instanceof Error ? error.message : String(error),
                        userId: user?.id
                    });
                    ...
                    Toast.show({
                        type: 'error',
                        text1: 'Error',
                        text2: 'Failed to prepare subscription options.',
                        position: 'bottom'
                    });
...
const Learn: React.FC = () => {
    ...
    const continueToNext = params.continueToNext === 'true';
    ...
    const handleNextVideo = useCallback(async () => {
        ...
        try {
            ...
            if (!isSubscribed && !user?.is_pro) {
                ...
                if (!hasAvailableVideos) {
                    ...
                    // Immediately reset the gesture state to hide the next video indicator
                    setGestureState({ isActive: false, gestureY: 0, direction: null });
                    
                    // Pause the video before showing paywall
                    if (videoState.playing && playerRef.current) {
                        setVideoState(prev => ({ ...prev, playing: false }));
                    }
                    
                    // Use inline showSuperwall instead of navigating to paywall route
                    const paywallShown = await showSuperwall(user, true, async () => {
                        // Continue with the next video after successful purchase
                        logEvent('PAYWALL_SUCCESS_CONTINUE_TO_NEXT', { 
                            userId: user?.id
                        });
                        
                        // Let the normal flow of this function continue to load the next video
                    });
                    
                    if (!paywallShown) {
                        logEvent('PAYWALL_NOT_SHOWN', { 
                            userId: user?.id
                        });
                        return;
                    }
                    
                    // Check if the user became pro after paywall interaction
                    if (user?.is_pro) {
                        logEvent('USER_NOW_PRO_CONTINUING', { userId: user?.id });
                        // Continue with the video transition since user is now pro
                    } else {
                        logEvent('USER_STILL_NOT_PRO', { userId: user?.id });
                        return; // Don't proceed if user is still not pro
                    }
                }
        ...
        videoState.playing, // Add videoState.playing for pause condition
        playerRef // Add playerRef for video control
    ]);
    ...
    const panResponder = useMemo(() => PanResponder.create({
        ...
        onPanResponderRelease: (_, gestureState) => {
            ...
            if (gestureState.dy < -80) {
                // Check if user is eligible for next video before showing the indicator
                const checkEligibilityAndHandleNextVideo = async () => {
                    try {
                        // If no user, we can't check eligibility, so just proceed with the normal flow
                        if (!user) {
                            logEvent('NO_USER_FOR_ELIGIBILITY_CHECK', {});
                        ...
                        // Only show the next video indicator if the user has videos available or is pro
                        const { isPro: isSubscribed } = await checkSubscriptionStatus();
                        
                        user.is_pro = isSubscribed;
                        
                        // Free users need to have videos available
                        if (!isSubscribed && !user.is_pro) {
                            const hasAvailableVideos = await checkFreeVideosAvailable(user.id);
                            
                            if (!hasAvailableVideos) {
                                // Don't show the indicator, just show the paywall directly
                                logEvent('MAX_FREE_VIDEOS_REACHED_FROM_GESTURE', { userId: user.id });
                                
                                // Reset gesture state so no indicator shows
                                setGestureState({ isActive: false, gestureY: 0, direction: null });
                                ...
                                // Show paywall
                                await showSuperwall(user, true);
                                
                                // Check if the user became pro after paywall interaction
                                if (user?.is_pro) {
                                    logEvent('USER_NOW_PRO_CONTINUING_FROM_GESTURE', { userId: user.id });
                                    // Continue with the next video transition since user is now pro
                                    setGestureState({ 
                                        isActive: false, 
                                        gestureY: Math.abs(gestureState.dy) * 1.2,
                                        direction: 'up' 
                        ...
                        // If we get here, user is eligible for next video, show the indicator and proceed
                        // Set the gesture state for the animation
                        setGestureState({ 
                            isActive: false, 
                            gestureY: Math.abs(gestureState.dy) * 1.2,
                        ...
                        // On error, reset gesture state and log error
                        setGestureState({ isActive: false, gestureY: 0, direction: null });
                        logEvent('ELIGIBILITY_CHECK_ERROR', { 
                            error: error instanceof Error ? error.message : String(error),
                            userId: user?.id
                        });
                    }
                ...
                checkEligibilityAndHandleNextVideo();
            } 
        ...
        user, checkSubscriptionStatus, checkFreeVideosAvailable, showSuperwall, videoState.playing, playerRef]);
    ...
    // In the Learn component, add this after other useEffect calls, before the return statement

    // Check URL parameters on mount to see if we should continue to next video
    useEffect(() => {
        if (continueToNext && user && state.currentLesson && !state.isTransitioning) {
            // Clear the parameter from the URL to prevent loops
            router.replace('/(tabs)/learn');
            
            // Wait a short time for the UI to stabilize
            setTimeout(() => {
                logEvent('CONTINUING_TO_NEXT_VIDEO_AFTER_PURCHASE', {
                    userId: user?.id,
                    currentLessonId: state.currentLesson?.id
                });
                
                // Trigger the next video transition
                handleNextVideo();
            }, 500);
        }
    }, [continueToNext, user, state.currentLesson, state.isTransitioning]);
