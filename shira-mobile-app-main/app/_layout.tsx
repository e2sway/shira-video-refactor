    ...
    useEffect(() => {
        const initServices = async () => {
            try {
                ...
                // Once RevenueCat is initialized, initialize Superwall with our purchase controller
                try {
                    const superwallInitialized = await initializeSuperwall();
                    console.log(`RootLayout: Superwall initialized ${superwallInitialized ? 'successfully' : 'with errors'}`);
                } catch (superwallError) {
                    console.error("RootLayout: Error initializing Superwall:", superwallError);
                    // Continue regardless of Superwall initialization status
                }
            } catch (error) {
        ...
        if (!isRevenueCatInitialized()) {
            ...
            setRevenueCatInitialized(true);
            // Try to initialize Superwall even if RevenueCat was already initialized
            initializeSuperwall().catch(error => {
                console.error("RootLayout: Error initializing Superwall with existing RevenueCat:", error);
            });
        }
    }, []);
