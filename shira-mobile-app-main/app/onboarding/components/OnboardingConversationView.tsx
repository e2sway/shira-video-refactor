...
import { colors } from '../styles'; // Import the colors from styles
...
interface ConversationalViewProps {
    ...
    onContinue?: () => void; // Add callback for navigating to next step
}
...
// Storage key for onboarding interaction limit
const INTERACTION_LIMIT_REACHED_KEY = '@shira_onboardingInteractionLimitReached';
// Add a new constant for the disabled conversation state key
const CONVERSATION_DISABLED_KEY = '@shira_conversationDisabled';

// Define interaction limits
const MAX_TTS_ACTIONS = 5;
const MAX_USER_MESSAGES = 7;
// Add a constant for the timer duration (1 minute and 15 seconds in milliseconds)
const CONVERSATION_TIME_LIMIT = 75000; // 1m15s in milliseconds

// Move the styles constant declaration to before any of its usage
const styles = StyleSheet.create({
    ...
    expandedContainer: {
        borderRadius: 0, // Remove border radius since modalContent already has it
        overflow: 'hidden',
        backgroundColor: 'transparent', // Make it transparent to let modalContent background show
        shadowColor: 'transparent', // Remove shadow as modalContent already has it
        shadowOpacity: 0,
        elevation: 0,
        flex: 1,
    ...
    modalContainer: {
        ...
        alignItems: 'center',
        backgroundColor: 'transparent', // Changed from rgba(0, 0, 0, 0.5) for better background fit
        paddingBottom: Platform.OS === 'ios' ? 10 : 5,
    },
    modalContent: {
        width: '92%',
        ...
        elevation: 15,
        marginBottom: 20,
        backgroundColor: colors.background, // Ensure the content has a background color
    },
    blurOverlay: {
        position: 'absolute',
        top: 0,
        ...
        right: 0,
        bottom: 0,
        backgroundColor: 'rgba(0, 0, 0, 0.5)', // Move background color here for consistent rendering
    },
    headerGradient: {
        position: 'absolute',
        top: 0,
    ...
    // Add styles for the move on overlay
    moveOnOverlayContainer: {
        position: 'absolute',
        top: 0,
        left: 0,
        right: 0,
...
const ConversationalView: React.FC<ConversationalViewProps> = ({ conversationalData, onContinue }) => {
    // State
    ...
    // Add new state for tracking interaction counts
    const [ttsActionCount, setTtsActionCount] = useState(0);
    const [userMessageCount, setUserMessageCount] = useState(0);
    const [interactionLimitReached, setInteractionLimitReached] = useState(false);
    const [moveOnOverlayVisible, setMoveOnOverlayVisible] = useState(false);
    const moveOnOpacity = useRef(new Animated.Value(0)).current;
    
    // Add a ref to store the timer
    const timerRef = useRef<NodeJS.Timeout | null>(null);
    // Add state to track if timer is active
    const [isTimerActive, setIsTimerActive] = useState(false);

    // Add state to track if conversation feature is disabled
    const [isConversationDisabled, setIsConversationDisabled] = useState(false);

    // Add effects to check if the user has seen each tutorial type and if interaction limit was reached
    useEffect(() => {
        const checkTutorialStatus = async () => {
            try {
                ...
                const conversationDisabledStatus = await AsyncStorage.getItem(CONVERSATION_DISABLED_KEY);
                if (conversationDisabledStatus === 'true') {
                    console.log('Conversation is disabled from previous session');
                    setIsConversationDisabled(true);
                } else {
                    console.log('Conversation is not disabled');
                    setIsConversationDisabled(false);
                }
                
                // Not checking for interaction limit status anymore
                // Since we want this to reset on app reload
                ...
                setInitialized(true);
                console.log('Status check initialized');
            }
        };
    ...
    const processRecording = async (uri: string) => {
        ...
        try {
            ...
            // Remove the message count check - no longer triggering move on overlay based on message count
    ...
    // Update the toggleExpand function to check the interaction limit
    const toggleExpand = () => {
        ...
        if (!isExpanded) {
            ...
            // When expanding, animate the icon first, then show the modal - same for both cases
            Animated.sequence([
                ...
                // Set expanded state - useEffect will handle showing move on overlay if needed
                setIsExpanded(true);
                
                // The timer will be started by the useEffect hook when isExpanded becomes true
                ...
                Animated.parallel([
                    ...
                    // Only start tutorial if interaction limit not reached
                    if (!interactionLimitReached) {
            ...
            // Hide move on overlay if visible
            if (moveOnOverlayVisible) {
                hideMoveOnOverlay();
            }
            ...
            if (timerRef.current) {
                ...
                console.log('Cleared conversation time limit timer on modal close');
            }
    ...
    // Add function to check and handle interaction limit
    const checkAndHandleInteractionLimit = async () => {
        if (ttsActionCount >= MAX_TTS_ACTIONS) {
            // Set the interaction limit reached flag
            setInteractionLimitReached(true);
            ...
            if (timerRef.current) {
                ...
                timerRef.current = null;
                setIsTimerActive(false);
                console.log('Cleared conversation time limit timer due to TTS limit reached');
            }
            
            // Intentionally not saving to AsyncStorage to allow the limit to reset when app reloads
            // This ensures users can experience the full interaction flow again after restarting the app
            console.log('Interaction limit reached (TTS actions) - will be reset on app reload');
            
            // Show the move on overlay
            showMoveOnOverlay();
        }
    };

    // Add function to show the move on overlay
    const showMoveOnOverlay = () => {
        setMoveOnOverlayVisible(true);
        
        // Animate in the overlay
        Animated.timing(moveOnOpacity, {
            toValue: 1,
            duration: 300,
            useNativeDriver: true,
            easing: Easing.out(Easing.cubic)
    ...
    // Add function to hide the move on overlay
    const hideMoveOnOverlay = () => {
        ...
        Animated.timing(moveOnOpacity, {
            ...
            setMoveOnOverlayVisible(false);
        });
    };
    
    // Update function to handle continuing to next step - now just closes the modal
    const handleMoveOn = () => {
        // Add haptic feedback
        Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Medium);
        ...
        if (timerRef.current) {
            ...
            timerRef.current = null;
            setIsTimerActive(false);
            console.log('Cleared conversation time limit timer on move on');
        }
        
        // Close the modal only - don't navigate to next step
        toggleExpand();
        
        // Disable the conversation feature
        setIsConversationDisabled(true);
        
        // Save disabled state to AsyncStorage
        AsyncStorage.setItem(CONVERSATION_DISABLED_KEY, 'true')
            .then(() => console.log('Conversation disabled state saved to AsyncStorage'))
            .catch(err => console.error('Error saving conversation disabled state:', err));
        
        console.log('Modal closed and conversation feature disabled');
    };
    ...
    // Update the handleSpeechPress function to track TTS actions
    const handleSpeechPress = (text: string, itemId: string) => {
        // If interaction limit is reached, show the overlay
        if (interactionLimitReached) {
            showMoveOnOverlay();
            return;
        }
        ...
        // Increment TTS action count
        const newTtsCount = ttsActionCount + 1;
        setTtsActionCount(newTtsCount);
        
        // Check if limit is reached
        if (newTtsCount >= MAX_TTS_ACTIONS) {
            checkAndHandleInteractionLimit();
        }
    ...
    // Add a new useEffect to ensure the move on overlay is shown when expanded and interaction limit reached
    useEffect(() => {
        // When both expanded state is true and interaction limit is reached, show the overlay
        if (isExpanded && interactionLimitReached) {
            console.log('Showing move on overlay due to expanded state + interaction limit reached');
            // Using setTimeout to ensure the modal is fully rendered before showing the overlay
            setTimeout(() => {
                showMoveOnOverlay();
            }, 100);
        }
    }, [isExpanded, interactionLimitReached]);

    // Fix the renderMoveOnOverlay function to ensure it's visible
    const renderMoveOnOverlay = () => {
        if (!moveOnOverlayVisible) return null;
        
        return (
            <Animated.View 
                style={[
                    styles.moveOnOverlayContainer,
                    { opacity: moveOnOpacity }
                ]}
                pointerEvents="auto" // Ensure overlay captures touch events
            >
                <BlurView intensity={15} style={styles.moveOnBlur} tint="dark">
                    <View style={styles.moveOnBox}>
                        <Text style={styles.moveOnTitle}>Great Exploration!</Text>
                        <Text style={styles.moveOnText}>
                            You've gotten a good taste of talking with Shira. In the app, this conversation is endless.
                        </Text>
                        <TouchableOpacity
                        ...
                        >
                            <LinearGradient
                                ...
                                style={styles.moveOnButtonGradient}
                            >
                                <Text style={styles.moveOnButtonText}>Close</Text>
                            </LinearGradient>
                        </TouchableOpacity>
                    </View>
                </BlurView>
    ...
    // Add back the missing functions for tutorial handling
    ...
    const handleStartConversation = () => {
        // Do nothing if conversation is disabled
        if (isConversationDisabled) {
            console.log('Conversation is disabled - ignoring start request');
            return;
        }
    ...
    // Add a new useEffect to clean up on unmount
    useEffect(() => {
        ...
        return () => {
            ...
            if (timerRef.current) {
                clearTimeout(timerRef.current);
                timerRef.current = null;
                console.log('Cleared conversation time limit timer on unmount');
            }
            
            // Clear the interaction limit reached state from AsyncStorage when component unmounts
            // This ensures the limit tracking doesn't persist across app reloads
            AsyncStorage.removeItem(INTERACTION_LIMIT_REACHED_KEY)
                .then(() => console.log('Interaction limit state cleared on unmount'))
                .catch(err => console.error('Error clearing interaction limit state:', err));
        };
    }, []); // Empty dependency array means this runs only on mount/unmount

    // Add useEffect to ensure interaction counts are reset on mount
    useEffect(() => {
        // Reset interaction counts when component mounts
        setTtsActionCount(0);
        setUserMessageCount(0);
        setInteractionLimitReached(false);
        console.log('Interaction counts reset on component mount');
    }, []); // Empty dependency array means this runs only once on mount

    // Start the timer when the conversation view is opened or expanded
    useEffect(() => {
        // Only start the timer when the modal is expanded and we have conversation data
        if (isExpanded && conversationalData && !interactionLimitReached) {
            // Only start a new timer if one isn't already running
            if (!timerRef.current) {
                console.log('Starting conversation time limit timer: 1m15s');
                setIsTimerActive(true);
                
                // Create a timer that will trigger the move on overlay after 1m15s
                timerRef.current = setTimeout(() => {
                    console.log('Conversation time limit reached');
                    // Set the interaction limit reached flag
                    setInteractionLimitReached(true);
                    // Show the move on overlay
                    showMoveOnOverlay();
            ...
            // Clean up the timer if any of the conditions change
            if (timerRef.current) {
                clearTimeout(timerRef.current);
                timerRef.current = null;
                setIsTimerActive(false);
                console.log('Conversation time limit timer cleared');
            }
        }
        ...
        return () => {
            if (timerRef.current) {
                ...
                timerRef.current = null;
                setIsTimerActive(false);
                console.log('Conversation time limit timer cleaned up');
            }
        };
    }, [isExpanded, conversationalData, interactionLimitReached]); // Removed isTimerActive from dependencies
                ...
                <View style={styles.container}>
                    ...
                    <View style={styles.centeredButtonContainer}>
                        <View style={styles.buttonBlurOutline}>
                            <TouchableOpacity
                                style={[
                                    styles.conversationButtonOutline,
                                    isConversationDisabled && styles.conversationButtonDisabled
                                ]}
                                onPress={handleStartConversation}
                                activeOpacity={isConversationDisabled ? 1 : 0.8} // No opacity change when disabled
                                disabled={isConversationDisabled}
                            >
                                <LinearGradient
                                    colors={
                                        isConversationDisabled 
                                        ? ['rgba(150, 150, 150, 0.3)', 'rgba(100, 100, 100, 0.3)'] // Grayed out colors
                                        : ['rgba(90, 81, 225, 0.8)', 'rgba(225, 81, 144, 0.8)']
                                    }
                                    start={{ x: 0, y: 0 }}
                                    end={{ x: 1, y: 1 }}
                                    style={styles.conversationButtonGradient}
                                >
                                    {/* Animated sheen effect - only visible during animation and when not disabled */}
                                    {!isConversationDisabled && (
                                    <Animated.View 
                                        style={[
                                    ...
                                    <View style={styles.buttonContent}>
                                        {isButtonLoading ? (
                                            ...
                                            <Text style={[
                                                ...
                                                isConversationDisabled && styles.conversationButtonTextDisabled
                                            ]}>
